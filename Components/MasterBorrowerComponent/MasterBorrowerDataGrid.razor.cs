...
{
  public partial class MasterBorrowerDataGrid
  {
    #region Service
    ...
    #region Class field
    private const string BASE_ROUTE = "/setting/borrower";
    #endregion

    #region LoadData
    protected async Task<List<JsonObject>?> LoadData(DataGridLoadArgs args)
    {
      var res = await IFINTEMPLATEClient.GetRows<JsonObject>("MasterBorrower", "GetRows", new 
      { 
        args.Keyword, 
        args.Offset, 
        args.Limit 
    ...
    private void Add()
    {
      NavigationManager.NavigateTo($"{BASE_ROUTE}/add");
    }
    #endregion
    ...
    private async Task Delete()
    {
      ...
      if (userConfirm == true)
      {
        ...
        await IFINTEMPLATEClient.Delete("MasterBorrower", "DeleteByID", idList);
